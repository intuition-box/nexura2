...
import { quest } from "@/models/tasks.model";
...
export const createQuest = async (req: GlobalRequest, res: GlobalResponse) => {
  try {
    ...
    if (!success) {
      res.status(BAD_REQUEST).json({ error: "send the correct data required to create a quest" });
      return;
    }

    const newQuest = new quest(req.body);
    if (newQuest.category !== "daily") {
      await newQuest.save()
    } else {
    ...
    res.status(OK).json({ message: "quest task created!" });
  } catch (error) {
    logger.error(error);
