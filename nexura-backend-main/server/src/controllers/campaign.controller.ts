...
export const claimCampaignRewards = async (
	...
	try {
		...
		if (!userToReward) {
			res
				.status(NOT_FOUND)
				.json({ error: "id associated wit user is invalid" });
			return;
		}
		...
		if (!completedCampaign.tasksCompleted) {
			res
				.status(FORBIDDEN)
				.json({
					error: "all tasks must be completed before rewards can be claimed",
				});
			return;
		}
		...
		const trustTokens = campaignToClaimRewards.reward?.tokenPerUser as number;
		...
		res
			.status(INTERNAL_SERVER_ERROR)
			.json({ error: "error claiming campaign task" });
	}
};
