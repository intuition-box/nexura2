// Quick script to test Twitter OAuth1 request_token step using .env.local
...
async function run() {
  // Trim env values in case of accidental whitespace/newlines in .env.local
  ...
  // Mask sensitive parts of the Authorization header for logging
  ...
  const res = await fetch(request_data.url, { method: 'POST', headers: { ...headers, 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ oauth_callback: callback }) });
