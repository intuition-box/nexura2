...
try {
  ...
  if (explicit) {
    ...
    try { console.log('[preloadEnv.cjs] loaded DOTENV_CONFIG_PATH=', explicit); } catch (e) {}
  } else {
    if (!process.env.DATABASE_URL) {
      ...
      for (const f of tryFiles) {
        ...
        if (p) {
          ...
          try { console.log('[preloadEnv.cjs] loadedFrom=', p); } catch (e) {}
          break;
        }
      }
    } else {
      try { console.log('[preloadEnv.cjs] DATABASE_URL already present in environment'); } catch (e) {}
    }
  }
} catch (e) {
